  ...
  return Promise.all(currentQueues.map(queue => {
    ...
    return ch.consume(queue, msg => {
      try {
        ...
        const badHeadersErr = badHeaders(rabQ, msg);
        if (badHeadersErr) {
          // Should reject the message in the future. For now, just a error event.
          rabQ.emit('log', {
            level: 'error',
            token: getTokenFromMessage(msg),
            msg: `Incorrect header in message` + badHeadersErr,
            err: badHeadersErr
          });
        }
...
function badHeaders(rabQ, msg) {
  if (!rabQ.requiredHeaders) {
    return null;
  }
  if (!msg.properties || !msg.properties.headers) {
    return 'No headers';
  }
  const missingHeaders = rabQ.requiredHeaders.filter(header => {
    return !msg.properties.headers[header];
  });
